...
onthefly_map2tod_kernel(
    T *tod,
    const T *lmap,
    const T *pointing_basis,
    const T *pointing_coeffs,
    ...
    bool partial_pixelization)
{
	...
	while (iterator.get_cl()) {
		...
		continue;
            ...
            // The on-the-fly pointing needs actual detector and time
            // indices, which is a bit cumbersome here
            long di = s/nt;
            long ti = s%nt; // would s-det*t be faster? It has a data dependency
    ...
    bool debug)
{
    ...
    check_onthefly_pointing(pointing_basis, pointing_coeffs, plan.nsamp, "launch_map2tod", true);
    ...
    if (debug) {
	onthefly_map2tod_kernel<T,W,true> <<< plan.pp.pointing_nblocks, {32,W}, shmem_nbytes >>>
	    (tod.data,                                  // T *tod [nsamp]
	...
	onthefly_map2tod_kernel<T,W,false> <<< plan.pp.pointing_nblocks, {32,W}, shmem_nbytes >>>
	    (tod.data,                                  // T *tod
	     local_map.data,                            // const T *lmap
	     pointing_basis.data,                       // const T *pointing_basis [nbasis,nt]
	     pointing_coeffs.data,                      // const T *pointing_coeffs [{y,x,alpha},ndet,nt]
...
#define INSTANTIATE(T) \
    template void launch_planned_onthefly_map2tod( \
	ksgpu::Array<T> &tod,	\
	const ksgpu::Array<T> &local_map, \
	const ksgpu::Array<T> &pointing_basis, \
	const ksgpu::Array<T> &pointing_coeffs, \
	const LocalPixelization &local_pixelization, \
	const PointingPlan &plan, \
	bool partial_pixelization, \
	bool debug)
